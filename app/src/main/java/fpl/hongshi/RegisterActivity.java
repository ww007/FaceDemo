...
public class RegisterActivity extends Activity implements SurfaceHolder.Callback, DialogInterface.OnClickListener {
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == -1) {
//            for (int i=0;i<500;i++){
//                AFR_FSDKFace mface=new AFR_FSDKFace();
//                byte[]a={(byte) i, (byte) i, (byte) i, (byte) i};
//                mface.setFeatureData(a);
//                Log.i(TAG, "insert: "+a);
//                helper.insert(mEditText.getText().toString()+i, mface.getFeatureData());
//            }
//			helper.insert(mEditText.getText().toString(), Base64.encodeToString(mAFR_FSDKFace.getFeatureData(), Base64.DEFAULT));
            helper.insert(mEditText.getText().toString(), mAFR_FSDKFace.getFeatureData());
